...
export function ApiTokenPanel() {
  ...
  const create = async event => {
    ...
    try {
      ...
      setSecret(result.secret); setName(''); setCreating(false); await load();
  ...
  return <section className="data-card reliability-card automation-card">
    <div className="reliability-head"><div><span className="eyebrow">AUTOMATION API</span><h2>API 令牌</h2><p>供 CLI 和脚本使用；密钥只显示一次，服务端仅保存摘要。<a href="api/v1/openapi.json" target="_blank" rel="noreferrer">查看 OpenAPI 合同</a></p></div><button className="button primary" type="button" disabled={busy || !!secret} aria-expanded={creating} onClick={() => { setCreating(value => !value); setError(''); setMessage(''); }}>{creating ? '取消创建' : '创建令牌'}</button></div>
    ...
    {creating && <form className="recovery-form automation-create-form" onSubmit={create}>
