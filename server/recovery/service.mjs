  ...
  return value.map(item => {
    ...
    const nodes = Array.isArray(item.nodes) ? item.nodes.map(node => {
      ...
      return {
        ...node,
        id: nodeId,
        stableKey: requiredText(node.stableKey, '节点稳定键', 2048),
...
function validatePayload(payload) {
  ...
  if (!ports || typeof ports !== 'object' || Array.isArray(ports) || typeof ports.ports !== 'object') throw new Error('恢复数据中的端口配置无效')
  return { subscriptions, ports }
}

export class RecoveryService {
  constructor({ subscriptionStore, exportPorts, restorePorts, suspend, resume, appVersion = 'unknown' } = {}) {
  ...
  async restore(recoveryPackage, password) {
