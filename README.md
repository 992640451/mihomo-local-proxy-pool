...
| 固定入口 | 自动调度 | 看得见、可验证 |
| --- | --- | --- |
| 节点变化时，应用里的代理地址不用改 | 支持故障切换、连接轮询和会话固定等 6 种策略 | 浏览器管理端口池，一键检测监听与出口分布 |
...
想使用按连接轮询时，选择至少两个节点，并把策略改成 **按连接轮询**。轮询只对新连接生效，已建立的 TCP 连接不会在节点之间迁移。
...
| 按连接轮询 | 希望新连接分散到多个节点 | 每个新连接依次使用不同健康节点 |
| 会话轮换 | 浏览器配置文件一次使用期间必须固定节点 | 开始时选择健康节点，结束后下次开始再切换 |

自动策略只使用通过健康检查的节点。会话轮换仅支持受管内置 Mihomo，未开始会话时端口拒绝流量；活跃会话即使节点故障也不会自动换点。节点不足、端口超出范围或订阅更新移除了节点时，服务会拒绝保存并说明原因。完整操作见 [会话轮换使用指南](docs/BROWSER_SESSIONS.md)。
...
- 端口监听检测与多连接出口分布验证。
- 浏览器配置文件级会话轮换：一次使用固定节点，下次开始切换；支持网页绑定并一键启动 Roxy，也提供普通浏览器命令行启动器。参见 [会话轮换使用指南](docs/BROWSER_SESSIONS.md)。
...
- “系统设置”可以下载口令加密恢复包；恢复是整体替换，包中没有的资源会被删除，必须先预检。恢复包不包含管理认证、登录会话、活动代理使用会话、Roxy API Key/窗口绑定、API 令牌、审计、检测历史或检测调度设置，也不修改目标机器的端口映射；恢复前必须结束所有代理使用会话。
...
- `proxy-session-data` 保存订阅、登录会话、代理使用会话、浏览器接入配置、API 令牌摘要、审计、检测历史/设置和端口池；`proxy-mihomo-data` 保存 Mihomo 运行配置。
...
| 浏览器会话轮换与 Roxy | [阅读](docs/BROWSER_SESSIONS.md) | [Read](docs/BROWSER_SESSIONS_EN.md) |
