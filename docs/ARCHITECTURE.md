...
- `server/proxySessions.mjs` 与 `server/proxySessionStore.mjs` 维护代理使用会话状态机和持久化游标；`server/browser/` 只负责浏览器适配与加密绑定，不参与节点轮换算法。
...
订阅、登录会话、代理使用会话、浏览器接入、API 令牌、审计和可观测性数据库使用 SQLite `PRAGMA user_version` 记录结构版本。迁移必须遵守：
...
迁移备份只用于升级失败时的紧急回退，不能替代停服后的数据目录备份或应用的加密配置备份。配置恢复包不包含认证、登录会话、活动代理使用会话、浏览器 API 密钥/绑定、令牌、审计或可观测性历史/设置。

## 代理使用会话边界

`session-round-robin` 在未开始时生成只含 `REJECT` 的策略组。开始操作先阻断并清理旧连接，再按持久化游标寻找与上次不同的健康节点；只有 Controller 确认策略组被限制到唯一节点后才提交 `active`。结束操作重新阻断、清理连接并提交 `ended`。开始标识用于幂等重试，旧会话 ID 不能结束较新的会话。

活动绑定与游标必须跨应用和核心重启保存；中断的过渡状态转为 `recovery-required`，不能推测浏览器已经退出。恢复配置只携带端口策略，不携带活动会话或浏览器绑定，并在存在未结束会话时拒绝执行。浏览器适配器只能在确认目标窗口关闭后结束准确的会话；状态不确定时保留绑定。

## 版本化 API 与恢复边界

`/api/v1` 的 21 个操作由 `server/automation/contract.mjs` 定义，应用版本与 API v1 独立演进。新增响应字段允许向后兼容，破坏性变更需新 API 主版本；未版本化接口不接受 API 令牌。权限与 CLI 约定见 [自动化指南](../AUTOMATION.md)，会话状态机的用户约定见 [会话轮换指南](BROWSER_SESSIONS.md)。
