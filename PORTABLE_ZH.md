...
便携数据位于解压根目录下的 `data` 目录，而非 `bin` 内。复制前先停止服务，保证订阅、加密密钥、登录会话、代理使用会话、浏览器接入配置、API 令牌、审计、检测历史与端口池一致。将新包解压到新目录，把备份的 `data` 复制进去，再启动新版本；不要同时运行两份实例。在验证新实例前保留旧目录和备份，不要让旧程序直接读取已被新版本升级的数据库。更新不得覆盖原 `data`。

可在「系统设置」下载口令加密配置恢复包，其中包含订阅与端口池，但不包含管理凭据、登录会话、活动代理会话、Roxy API Key/窗口绑定、API 令牌、审计、检测历史或检测调度设置。恢复前必须结束所有代理使用会话；恢复是整体替换，需先预检并使用有效签名计划，不会修改主机端口映射。
...
会话轮换和 Roxy 网页接入均支持便携部署，Roxy 本地 API 由管理器通过 `127.0.0.1` 访问。普通浏览器或脚本场景可使用 `bin/ppm.cmd launch`（Linux/macOS 使用 `./ppm launch`）；完整配置、密钥文件和异常恢复见 [会话轮换使用指南](docs/BROWSER_SESSIONS.md)。
